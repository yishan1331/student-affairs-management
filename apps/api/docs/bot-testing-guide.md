...
- [指令短別名對照](#指令短別名對照)
- [中英文關鍵字對照](#中英文關鍵字對照)
- [Vercel 部署（Webhook 模式）](#vercel-部署webhook-模式)
- [常見問題](#常見問題)
...
本地開發使用 polling 模式（Telegram long polling + Slack Socket Mode），Vercel 部署需切換為 webhook 模式。
...
```env
BOT_MODE=webhook
BOT_WEBHOOK_BASE_URL=https://your-app.vercel.app
TELEGRAM_BOT_TOKEN=你的telegram-bot-token
TELEGRAM_WEBHOOK_SECRET=自訂隨機字串（用於驗證 Telegram webhook 請求）
SLACK_BOT_TOKEN=xoxb-你的bot-token
SLACK_SIGNING_SECRET=你的signing-secret
# 不需要 SLACK_APP_TOKEN（webhook 模式不使用 Socket Mode）
```
...
# 先設定環境變數（或在 .env.development.local 中設定）
export BOT_WEBHOOK_BASE_URL=https://your-app.vercel.app
export TELEGRAM_WEBHOOK_SECRET=你的自訂secret
...
部署到 Vercel 後需要調整 Slack App 設定：

1. **關閉 Socket Mode**
   - 左側選單 **Socket Mode** → 關閉 **Enable Socket Mode**

2. **設定 Slash Commands Request URL**
   - 左側選單 **Slash Commands**
   - 編輯每個已註冊的指令，將 Request URL 設定為：
     ```
     https://your-app.vercel.app/api/bot/slack/events
     ```

3. **設定 Event Subscriptions**
   - 左側選單 **Event Subscriptions** → 開啟 **Enable Events**
   - Request URL 設定為：
     ```
     https://your-app.vercel.app/api/bot/slack/events
     ```
   - Slack 會自動發送 `url_verification` challenge 驗證

4. **重新安裝 App**
   - 如果更改了 scope 或設定，需要重新安裝 App 到 Workspace

### 本地 vs Vercel 模式差異

| 項目 | 本地（polling） | Vercel（webhook） |
|------|----------------|-------------------|
| BOT_MODE | `polling`（預設） | `webhook` |
| Telegram | Long polling | HTTP webhook |
| Slack | Socket Mode | HTTP webhook |
| SLACK_APP_TOKEN | 需要 | 不需要 |
| TELEGRAM_WEBHOOK_SECRET | 不需要 | 建議設定 |
| BOT_WEBHOOK_BASE_URL | 不需要 | 必須設定 |
| 速率限制 | 記憶體中持久 | 每次 cold start 重置 |

### 注意事項

- **Vercel 10s timeout**：所有 Bot 指令處理（DB 查詢 + 回覆）需在 10 秒內完成
- **Cold start**：首次請求可能較慢，Neon serverless PostgreSQL 已支援
- **速率限制**：記憶體中的 Map 在 serverless 環境每次 cold start 會重置，目前可接受
