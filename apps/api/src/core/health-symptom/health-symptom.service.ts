...
export class HealthSymptomService {
	...
	async findAll(query: Prisma.HealthSymptomFindManyArgs, userId: number) {
	...
	async count(where: Prisma.HealthSymptomWhereInput, userId: number) {
		return this.prisma.healthSymptom.count({
	...
	async update(id: number, dto: UpdateHealthSymptomDto, userId: number) {
	...
	async removeBatch(ids: number[], userId: number) {
		const result = await this.prisma.healthSymptom.deleteMany({
